# Auto-fill weekly rotation - runs Sunday 4 PM Pacific
...
Add-Content $LogFile "`n[$timestamp] Running weekly auto-fill..."

try {
    Set-Location $ProjectDir
    $env:DATABASE_URL = "postgres://admin:password123@localhost:15433/fantasy_db"
    $output = & go run ./cmd/auto_rotation --mode=weekly 2>&1
    $output | ForEach-Object { Write-Host $_; Add-Content $LogFile "  $_" }
    Add-Content $LogFile "[$timestamp] Weekly fill completed successfully"
} catch {
