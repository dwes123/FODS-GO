...
// GetUserOutbidPlayers returns players the user previously bid on but where another team now holds the highest bid.
func GetUserOutbidPlayers(db *pgxpool.Pool, userID string) ([]PendingBidPlayer, error) {
	ctx := context.Background()
	...
	query := `
		...
		LEFT JOIN teams t ON p.pending_bid_team_id = t.id
		WHERE p.fa_status = 'pending_bid'
		  AND p.pending_bid_team_id NOT IN (SELECT team_id FROM team_owners WHERE user_id = $1)
		  AND EXISTS (
