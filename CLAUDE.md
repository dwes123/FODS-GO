...
internal/
  handlers/              — HTTP handlers (one file per feature area)
    ...
    moves.go             — Roster moves (dynamic limits, SP limit, 40-man, 26-man, option, IL, DFA, trade block, rookie contract auto-assign, SP/RP position swap)
...
- **Pending actions table** — Commissioner dashboard shows Type, Player, Team, League, Details, Actions columns; league name fetched via JOIN on `leagues` table
...
1. **Extension Calculator** — WAR-based pricing on player profile (SP=3.3755, RP=5.0131, Hitter=2.8354, decay factors, $700K floor); submits years+AAV+WAR to pending_actions with year-by-year JSONB; auto-applies contract on approval; blocks players with >1 ARB year; finds first non-dollar contract year as start; 3-Year WAR value included in summary for commissioner review
...
16. **Email Notifications** — Brevo HTTP API via env vars (`BREVO_API_KEY`, `SMTP_FROM`); trade proposals email receiving team; DigitalOcean blocks SMTP ports so HTTP API is used instead
...
- **My Open Bids Page** — `/bids/my-bids` shows only the logged-in user's active bids (where `pending_bid_team_id` matches any of their teams via `team_owners`); `GetUserOpenBids()` in `store/bids.go` reuses `PendingBidPlayer` struct; columns: Player, Position, League, Team, Bid Points, Years, AAV, Time Remaining, Action; live countdown timers with color coding; empty state when no bids; nav link before "Pending Bids"; **Outbid Tracker** section below active bids shows players the user previously bid on during the current auction but was outbid; `GetUserOutbidPlayers()` queries `bid_history` JSONB for entries matching user's teams with `history_timestamp >= bid_end_time - 7 days` (scoped to current auction cycle only — old bids from prior years don't resurface; uses `bid_end_time` not `bid_start_time` because `bid_start_time` updates to the latest bidder's time); muted row styling with orange "Re-Bid" button; players disappear automatically when `fa_status` changes from `pending_bid`
...
- **Free Agents Pagination** — `/free-agents` now uses page-based navigation (50 per page) instead of a flat `LIMIT 50`; `CountFreeAgents()` in `store/players.go` provides total count; handler parses `page` query param, calculates offset; pagination bar shows "Showing X–Y of Z players" with Prev/Next links; all filters (`league_id`, `q`, `pos`, `ifa`, `milb`) preserved in pagination URLs; template helper functions `add`, `sub`, `mul`, `min` registered in `RenderTemplate` FuncMap; dark mode supported
- **SP/RP Position Swap** — `POST /roster/move/position-swap` lets team owners swap a pitcher between SP and RP; `PositionSwapRequest` struct adds `target_position` field; validates player is SP, RP, or P (generic pitcher); SP↔RP swaps have 14-day cooldown (checked via `roster_moves_log` JSONB for most recent "Position Swap" entry); "P" players can assign to SP or RP with no cooldown (first assignment); RP→SP on 26-man checks SP limit via `GetTeam26ManSPCount`; logs to roster moves and activity feed; swap button appears on player profile page (not roster table) — purple button(s) in a card below the meta grid, only visible to team owner; SP shows "Swap to RP", RP shows "Swap to SP", P shows both "Assign SP" and "Assign RP"
...
- **Audit Log** — All write tools log to `agent_audit_log` table (user_id, tool_name, tool_args JSONB, tool_result JSONB, created_at); 14 write tools tracked: `assign_player_to_team`, `release_player`, `update_player_name`, `update_team_balance`, `process_pending_action`, `process_registration`, `update_player_contract`, `add_dead_cap`, `dfa_player`, `set_league_date`, `update_bug_status`, `assign_team_owner`, `delete_player`, `update_player_bid`; read-only tools not logged; `migrations/024_agent_audit_log.sql`
...
- `migrations/024_agent_audit_log.sql` — Adds: `agent_audit_log` table (id UUID, user_id FK, tool_name TEXT, tool_args JSONB, tool_result JSONB, created_at TIMESTAMP) with indexes on user_id and created_at
