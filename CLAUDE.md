...
- **IFA signing:** International free agents use a separate ISBP-based signing flow — single dollar amount (no contract years/AAV/bid points); validates ISBP balance on bid, deducts on finalization; no contract written; IFA flag cleared on signing; player placed in non-40-man minors (`status_40_man = FALSE`, `status_26_man = FALSE`); `status_il` cleared to NULL on finalization
